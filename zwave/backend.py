...
Modified by Daniel Oliveira Paiva and Edin Mujkanovic for the IoT course.
...
class Backend():
    ...
    def __init__(self, **kwargs):
        ...
        self.queryStages = {
            ...
            "NodePlusInfo"          :  7, # Retrieve Z-Wave+ info and update device classes (Added by Edin et Daniel)
            "SecurityReport"        :  8, # Retrieve a list of Command Classes that require Security
            "ManufacturerSpecific2" :  9, # Retrieve manufacturer name and product ids
            "Versions"              : 10, # Retrieve version information
            "Instances"             : 11, # Retrieve information about multiple command class instances
            ...
            "Session"               : 16, # Retrieve session information (changes infrequently)
            "Dynamic"               : 17, # Retrieve dynamic information (changes frequently)
            "Configuration"         : 18, # Retrieve configurable parameter information (only done on request)
    ...
    def network_info(self):
        ...
        for k, node in nodes.items():
            percentage = round((self.queryStages[node.query_stage] / 19) * 100, 2)
            node = {
                k : {
...
class Backend_with_sensors(Backend):
    ...
    def get_sensor_readings(self, n):
        ...
        return {
                ...
                "controller": self.controller_name,
                "humidity": values["Relative Humidity"],
                "location": node.location,
                "luminance": values["Luminance"],
                "motion": values["Sensor"],
                "sensor": node.node_id,
                "temperature": f_to_c(int(values["Temperature"])),
                "updateTime": self._get_node_timestamp(node)
        }
    ...
    def set_sensors_parameter(self, index, value, size):
        ...
        for k,node in nodes.items():
            if self._is_sensor(node):
                values[k] = node.set_config_param(index, value, size)
...
class Backend_with_dimmers(Backend):
    ...
    def set_dimmer_level(self, n, value):
        ...
        node = self._lookup_dimmer_node(n)
        all_values = node.get_values("All","User","All","All").values()
        current_value = None
        for v in all_values:
            if(v.label == 'Level'):
                current_value = v.data
                v.data = value
                return [current_value, 'OK']
            else:
