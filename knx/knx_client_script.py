...
Modified by : Daniel Oliveira Paiva and Edin Mujkanovic
...
def compare_tunnelling_requests(tun_req1, tun_req2):
    """ Check if deux tunnelling_requests are the same. 
    :param tun_req1: the first tunneling request
    :param tun_req2: the second tunnelong request

    :return true if they are equals.
    """
    logger.debug('comparing two tunnelling requests')
    logger.debug(tun_req1)
    logger.debug(tun_req2)
    if((tun_req1.dest_addr_group  == tun_req2.dest_addr_group) & (tun_req1.channel_id==tun_req2.channel_id)  & (tun_req1.data_size==tun_req2.data_size) & (tun_req1.apci==tun_req2.apci) & (tun_req1.sequence_counter==tun_req2.sequence_counter) ):
        return True
    else:
        return False
...
def send_knx_request(
    ...
    logger.info('channel ID: {}'.format(conn_resp.channel_id))
    logger.info('reponse data endpoint: {}'.format(conn_resp.data_endpoint))

    if(conn_resp.status != 0):
            return False, 'Error'
    ...
    logger.debug('waiting for a connection state response (CONNECTION_STATE_RESPONSE -> conn_state_resp)')
    data_recv, addr = sock.recvfrom(buf_size)
    conn_state_resp = knxnet.decode_frame(data_recv)
    logger.info('reponse status: {}'.format(conn_state_resp.status))

    if(conn_state_resp.status != 0):
            return False, 'Error'

    # <- Tunneling request : client -> GW
    tun_req = knxnet.create_frame(
    ...
    sock.sendto(tun_req.frame, (gateway_ip, gateway_port))


    # <- Tunneling Ack : GW -> client
    logger.debug('waiting for a tunneling ack (TUNNELING_ACK -> tunn_ack)')
    data_recv, addr = sock.recvfrom(buf_size)
    tunn_ack = knxnet.decode_frame(data_recv)
    logger.info('reponse status: {}'.format(tunn_ack.status))

    if(tunn_ack.status != 0):
            return False, 'Error'

    # <- Tunneling request : GW -> client
    logger.debug('waiting for a tunneling request (TUNNELING_REQUEST -> tun_req2)')
    data_recv, addr = sock.recvfrom(buf_size)
    tun_req2 = knxnet.decode_frame(data_recv)
    logger.info('data service: {}'.format(tun_req2.data_service))

    if(tun_req2.data_service != 0x2e):
            return False, 'Error'


    if(compare_tunnelling_requests(tun_req, tun_req2) == False):
        return False, 'Error'

    # <- Tunneling Ack : Client -> GW

    tun_ack = knxnet.create_frame(
    ...
    # If it's a read request, wait the data
    result = 'Success'

    if(apci == 0):

        # <- Tunneling request : GW -> client
        logger.debug('waiting for a tunneling request (TUNNELING_REQUEST -> tunn_req)')
        data_recv, addr = sock.recvfrom(buf_size)
        tunn_req_2 = knxnet.decode_frame(data_recv)
        logger.info('data service: {}'.format(tunn_req_2.data_service))
        result = tunn_req_2.data

        if(tunn_req_2.data_service != 0x29):
            return False, 'Error'
    ...
    # <- Disconnect Response
    logger.debug('waiting for a disconnect response (DISCONNECT_RESPONSE -> dis_resp)')
    data_recv, addr = sock.recvfrom(buf_size)
    dis_resp = knxnet.decode_frame(data_recv)
    logger.info('data service: {}'.format(dis_resp.status))

    if(dis_resp.status != 0):
        return False, 'Error'
    else:
        return True, result
